...
it('keeps automatic iPhone intent while the phone disappears and returns without opening audio', async () => {
  await mount('agent-voice:auto-iphone')
  const availability = () => view.root.findByProps({ 'aria-label': 'iPhone availability' }).children.join('')
  expect(availability()).toContain('Ready: iPhone')
  devices.enumerateDevices.mockResolvedValue([])
  await act(async () => { devices.dispatchEvent(new Event('devicechange')) })
  expect(availability()).toContain('Waiting for macOS')
  devices.enumerateDevices.mockResolvedValue([{ kind: 'audioinput', deviceId: 'reconnected', label: 'iPhone' }])
  await act(async () => { devices.dispatchEvent(new Event('devicechange')) })
  expect(availability()).toContain('Ready: iPhone')
  expect(view.root.findByType('select').props.value).toBe('agent-voice:auto-iphone')
  expect(devices.getUserMedia).not.toHaveBeenCalled()
})
